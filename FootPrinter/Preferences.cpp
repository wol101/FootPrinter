...
const QString Preferences::applicationName("FootPrinter");
...
{
    std::cerr << "Preferences::Read() fileName = " << qUtf8Printable(fileName()) << "\n";
    LoadDefaults();
...
{
    QDomDocument doc("FootPrinterPreferences");
...
void Preferences::insert(const QString &key, const QVariant &value)
{
    ...
    else
    {
        if (static_cast<QMetaType::Type>(value.type()) != QMetaType::QByteArray)
            std::cerr << "Preferences::value " << value.typeName() << " " << qUtf8Printable(key) << " missing\n";
        item.key = key;
        item.display = false;
        item.label = key;
        item.value = value;
...
QVariant Preferences::qtValue(const QString &key, const QVariant &defaultValue)
{
    ...
    if (m_qtSettings->contains(newKey1))
    {
        ...
        if (m_qtSettings->contains(newKey2))
        {
            ...
            variant.convert(int(QVariant::nameToType(typeName.toUtf8()))); // convert to the type stored in the settings
        }
        else
        {
