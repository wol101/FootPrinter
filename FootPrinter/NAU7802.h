...
typedef enum
{
    NAU7802_CTRL1_GAIN = 0, // 0,1,2
    NAU7802_CTRL1_VLDO = 3, // 2,4,5
...
public:
    ...
    ~NAU7802();
    bool begin(const std::string& interface, unsigned int deviceID, bool initialize = true); //Create communications interface and initialize sensor
//    bool isConnected();                                    //Returns true if device acks at the I2C address
    ...
    bool cycleStart();                         //Start a conversion cycle
    ...
    uint8_t getRegister(uint8_t registerAddress, bool *err);             //Get contents of a register
    ...
    // wis - arduino utility functions
    uint64_t millis(); // time now in milliseconds
    void delay(uint64_t delayTime); // time to delay in milliseconds

    // threaded read functions
    void startReadThread();
    void stopReadThread();
    int32_t threadedValue();

private:
    std::unique_ptr<GPIO::I2C> _i2cPort; //This stores the required i2c port
    const uint8_t _deviceAddress = 0x2A; //Default unshifted 7-bit address of the NAU7802

    uint8_t m_storedSampleRate = uint8_t(NAU7802_SPS_10);
    ...
    std::atomic_bool m_readThreadStopRequested = {false};
    std::atomic_int32_t m_readThreadValue = {0};
    void readThread();
};
#endif
