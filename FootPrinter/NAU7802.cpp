...
// wis - edited to work on Raspberry Pi with CppGPIO
...
NAU7802::~NAU7802()
{
    stopReadThread();  // this command is safe if the thread is not running
}
...
bool NAU7802::begin(const std::string& interface, unsigned int deviceID, bool initialize)
{
    try
    {
        _i2cPort = std::make_unique<GPIO::I2C>(interface, deviceID);
    }
    catch (std::runtime_error &e)
    {
        std::cerr << "Error opening " << interface << " deviceID " << std::hex << deviceID << "\n";
        std::cerr << e.what() << "\n";
        return false;
    }
...
//    }
    ...
    if (initialize)
    {
        ...
        result &= setLDO(NAU7802_LDO_3V0); //Set LDO to 3.0V - must be at least 0.3V less than supply

        result &= setGain(NAU7802_GAIN_128); //Set gain to 128

        result &= setSampleRate(NAU7802_SPS_320); //Set samples per second to 320

        result &= setRegister(NAU7802_ADC, 0b00110000); //Turn off CLK_CHP. From 9.1 power on sequencing.
...
//Start a conversion cycle (only needed for synchronised reads)
bool NAU7802::cycleStart()
{
    if (setBit(NAU7802_PU_CTRL_CS, NAU7802_PU_CTRL)) return true;
    return (clearBit(NAU7802_PU_CTRL_CS, NAU7802_PU_CTRL));
}
...
bool NAU7802::setSampleRate(uint8_t rate)
{
    ...
    m_storedSampleRate = rate;

    bool err;
    uint8_t value = getRegister(NAU7802_CTRL2, &err);
    if (err) return err;
...
bool NAU7802::setLDO(uint8_t ldoValue)
{
    ...
    bool err;
    uint8_t value = getRegister(NAU7802_CTRL1, &err);
    if (err) return err;
...
{
    bool err;
    uint8_t revisionCode = getRegister(NAU7802_DEVICE_REV, &err);
    if (err) return 0;
...
{void ReadThread();
    bool err;
    uint32_t valueRaw = uint32_t(getRegister(NAU7802_ADCO_B2, &err)) << 16; //MSB
    if (err) return 0;
    valueRaw |= uint32_t(getRegister(NAU7802_ADCO_B1, &err)) << 8;          //MidSB
    if (err) return 0;
    valueRaw |= uint32_t(getRegister(NAU7802_ADCO_B0, &err));               //LSB
    if (err) return 0;
...
{
    bool err;
    uint8_t value = getRegister(registerAddress, &err);
    if (err) return err;
...
uint8_t NAU7802::getRegister(uint8_t registerAddress, bool *err)
{
    int reg = 0xff;
    *err = false;
    try
    {
        reg = _i2cPort->regread8(int(registerAddress));
    }
    catch (std::runtime_error &e)
    {
        std::cerr << e.what() <<"\n";
...
bool NAU7802::setRegister(uint8_t registerAddress, uint8_t value)
{
    try
    {
        _i2cPort->regwrite8(int(registerAddress), int(value));
    }
    catch (std::runtime_error &e)
    {
        std::cerr << e.what() <<"\n";
...
uint64_t NAU7802::millis()
{
    std::chrono::duration<double> timeAsDouble = std::chrono::system_clock::now().time_since_epoch();
    return uint64_t(timeAsDouble.count() * 1000.0 + 0.5);
}

void NAU7802::delay(uint64_t delayTime)
{
    std::this_thread::sleep_for(std::chrono::milliseconds(delayTime));
}

void NAU7802::startReadThread()
{
    m_readThread = std::make_unique<std::thread>(&NAU7802::readThread, this);

}

void NAU7802::stopReadThread()
{
    if (!m_readThread.get()) return;
    m_readThreadStopRequested = true;
    if (m_readThread->joinable()) m_readThread->join();
    m_readThread.release();
}

int32_t NAU7802::threadedValue()
{
    return m_readThreadValue;
}

void NAU7802::readThread()
{
    while (!m_readThreadStopRequested)
    {
        while (this->available() == false) {}
        m_readThreadValue = this->getReading();

        switch (m_storedSampleRate)
        {
        case NAU7802_SPS_320:
            std::this_thread::sleep_for(std::chrono::microseconds(3125));
            break;
    ...
    m_readThreadStopRequested = false;
}
